...
{
	const char* str;
	char name[SCRIPT_NAME_LENGTH], message[256];
...
BUILDIN_FUNC(instance_npcname)
{
	...
	if( instance_id && (nd = npc_name2id(str)) != NULL )
 	{
		static char npcname[SCRIPT_NAME_LENGTH];
...
/*==========================================
 * Cronus-Emulator [LyDe/Cainho]
 * Imprime uma mensagem e fecha o chat
 * mesclose; 
 *------------------------------------------*/
BUILDIN_FUNC(mesclose) {
	TBL_PC* sd = script_rid2sd(st);
	if(sd == NULL)
...
// <--- [zBuffer] List of mob control commands
	...
	BUILDIN_DEF(mesclose, "s"), /* Cronus Emulator - [LyDe/Cainho] */
	{NULL,NULL,NULL},
};
