...
{
	char npc_ev[EVENT_NAME_LENGTH];
	char card_slot[SCRIPT_NAME_LENGTH];
	int i, j, idx;
	int key_nameid = 0;
	int key_amount = 0;
	int key_refine = 0;
...
static void npc_parsename(struct npc_data* nd, const char* name, const char* start, const char* buffer, const char* filepath)
{
	...
	char newname[SCRIPT_NAME_LENGTH];
...
int do_init_npc(void)
{
	...
	ev_db = strdb_alloc((DBOptions)(DB_OPT_DUP_KEY|DB_OPT_RELEASE_DATA),2*SCRIPT_NAME_LENGTH+2+1);
	npcname_db = strdb_alloc(DB_OPT_BASE,SCRIPT_NAME_LENGTH);
