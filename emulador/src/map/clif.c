...
int clif_setip(const char* ip)
{
	...
	map_ip = host2ip(ip);
	if (!map_ip) {
		ShowWarning("Falha ao resolver o endreÁo do Map Server! (%s)\n", ip);
		return 0;
	}

	strncpy(map_ip_str, ip, sizeof(map_ip_str));
	ShowInfo("EndereÁo de IP do Map Server : '"CL_WHITE"%s"CL_RESET"' -> '"CL_WHITE"%s"CL_RESET"'.\n", ip, ip2str(map_ip, ip_str));
	return 1;
}
...
void clif_setbindip(const char* ip)
{
	...
	bind_ip = host2ip(ip);
	if (bind_ip) {
		ShowInfo("EndereÁo de Ip do Map Server Bind : '"CL_WHITE"%s"CL_RESET"' -> '"CL_WHITE"%s"CL_RESET"'.\n", ip, ip2str(bind_ip, ip_str));
	} else {
		ShowWarning("Falha ao resolver o endreÁo do Map Server! (%s)\n", ip);
	}
}
...
uint32 clif_refresh_ip(void)
{
	...
	if (new_ip && new_ip != map_ip) {
		map_ip = new_ip;
		ShowInfo("ResoluÁ„o de actualizaÁ„o IP [%s].\n", map_ip_str);
		return map_ip;
	}
	return 0;
}
...
/*==========================================
 * ÉXÉLÉãârè•ÉGÉtÉFÉNÉgÇëóêMÇ∑ÇÈ
 *------------------------------------------*/
...
void clif_parse_NpcSelectMenu(int fd,struct map_session_data *sd)
{
	...
	if( (select > sd->npc_menu && select != 0xff) || select == 0 )
	{
		...
		ShowWarning("SeleÁ„o inv·lida no menu do npc %d:'%s' - recebeu %d, valor v·lido È de [%d..%d] (AID:%d, CID:%d, nome:'%s')!\n", npc_id, (nd)?nd->name:"ID do NPC inv·lido", select, 1, sd->npc_menu, sd->bl.id, sd->status.char_id, sd->status.name);
...
void clif_parse_CloseVending(int fd, struct map_session_data* sd)
{
	...
	// FunÁ„o anti-exploit [Minos & LEOzinn]
 	iter = mapit_getallusers();
	for( pl_sd = (TBL_PC*)mapit_first(iter); mapit_exists(iter); pl_sd = (TBL_PC*)mapit_next(iter) )
	{
		if(pl_sd->cvender_id == sd->bl.id)
		{ 
		    clif_displaymessage(pl_sd->fd,"A loja foi fechada , vocÍ ser· recarregado para evitar perdas");
			pl_sd->cvender_id = 0;
            pc_setpos( pl_sd, pl_sd->mapindex , pl_sd->bl.x , pl_sd->bl.y , 0 );
		}
	}
	mapit_free(iter);
...
int clif_instance(struct block_list *bl, int type, ...)
{
	...
	switch(type){
	case 1: // R 02CB <Instance name>.S
		mes = va_arg(ap, const char*);
		WBUFW(buf,0) = 0x02CB;
		memcpy(WBUFP(buf,2), mes, packet_db[sd->packet_ver][RBUFW(buf,0)].len - 2); // more than 24 bytes looks scarry
		clif_send(buf, packet_db[sd->packet_ver][RBUFW(buf,0)].len, bl, SELF);
		//break;
	case 2: // R 02CC <Standby position>.W
		WBUFW(buf,0) = 0x02CC;
		WBUFW(buf,2) = va_arg(ap, int);
		clif_send(buf, packet_db[sd->packet_ver][RBUFW(buf,0)].len, bl, SELF);
		break;
	case 3: // R 02CD <Instance name>.S <time1>.L <time2>.L
		WBUFW(buf,0) = 0x02CD;
		memcpy(WBUFP(buf,2), va_arg(ap, const char*), 61); // more than 24 bytes looks scarry
		WBUFL(buf,63) = va_arg(ap, int);
		WBUFL(buf,67) = va_arg(ap, int);
		clif_send(buf, packet_db[sd->packet_ver][RBUFW(buf,0)].len, bl, SELF);
		break;
	case 4: // R 02CE <message ID>.L
		WBUFW(buf,0) = 0x02CE;
		WBUFL(buf,2) = va_arg(ap, int);
		clif_send(buf, packet_db[sd->packet_ver][RBUFW(buf,0)].len, bl, SELF);
		break;
	}
	va_end(ap);
