...
// Tradução Geral e organização feita por [Protimus]
...
int chat_createpcchat(struct map_session_data* sd, const char* title, const char* pass, int limit, bool pub)
{
	...
	if((battle_config.no_prt_chating) && (sd->bl.m == map_mapname2mapid("prontera.gat") && (sd->bl.x >= 148 && sd->bl.x <= 163 && sd->bl.y >= 139 && sd->bl.y <= 175)))
	{ // Não abrir Chats no meio da Rua [Minos]
		clif_displaymessage(sd->fd,"Você não pode abrir chats no meio da rua de Prontera.");
		return 0;
	}
	...
	{
		clif_displaymessage (sd->fd, "Você não pode criar salas de chat nessa área.");
		return 0;
	}
